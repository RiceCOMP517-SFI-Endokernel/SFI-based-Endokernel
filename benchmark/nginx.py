...
for i in range(0, len(variable.iv_nocet_paths)):
    ...
    if variable.iv_nocet_paths[i] == 'baseline':
        ...
        servercmd = "../nginx/objs/nginx -c ./nginx.conf -p /var/www"
        curbench = "nginx Beseline"
    else:
        filesuffix = "nginx_" + variable.iv_nocet_paths[i].split("/")[-2]
        servercmd = variable.iv_nocet_paths[i] + "libintravirt.so " + variable.glibcpath + " ../nginx/objs/nginx -c ./nginx.conf -p /var/www"
...
for i in range(0, len(variable.iv_cet_paths)):
    if variable.iv_nocet_paths[i] == 'baseline':
        filesuffix = "SFI_nginx_baseline"
        servercmd = "../nginx-sfi/objs/nginx -c ./nginx.conf -p /var/www"
        curbench = "nginx Beseline"
    # launch nginx
    else:
        filesuffix = "SFI_nginx_" + variable.iv_cet_paths[i].split("/")[-2]
        servercmd = variable.iv_cet_paths[i] + "libintravirt.so " + variable.cet_glibcpath + "  ../nginx-sfi/objs/nginx -c ./nginx.conf -p /var/www"
        curbench = "sfi-nginx " + variable.iv_cet_paths[i].split("/")[-2]
    resfilename = "./" + variable.resdir + "/" + filesuffix + ".csv"
    fp = open(resfilename + ".tmp", "wb")
    for size in datasizes:
        fp.write(size.encode())
